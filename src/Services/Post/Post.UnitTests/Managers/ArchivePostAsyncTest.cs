...
{
    public class ArchivePostAsyncTest
    {
        ...
        [Fact]
        public async Task Archive_post_throws_exception_when_author_not_found()
        {
            // Arrange
            ...
            // Assert
            await Assert.ThrowsAsync<AuthorNotFoundException>(() => postManager.ArchivePostAsync(fakePostId));
        }

        [Fact]
        public async Task Archive_post_throws_exception_when_post_not_found()
        {
            // Arrange
            var fakePostId = Guid.NewGuid();
            ...
            // Assert
            await Assert.ThrowsAsync<PostNotFoundException>(() => postManager.ArchivePostAsync(fakePostId));
        }

        [Fact]
        public async Task Archive_post_throws_exception_when_post_already_archived()
        {
            // Arrange
            var fakePostId = Guid.NewGuid();
            _fakePostEntity.Status = PostStatus.archived;
            _postRepository.Setup(x => x.ReadPostByIdAsync(It.IsAny<Guid>())).ReturnsAsync(_fakePostEntity);
            _authorManager.Setup(x => x.GetAuthorByIdAsync(It.IsAny<Guid>())).ReturnsAsync(_fakeAuthorResponse);
            ...
            // Assert
            await Assert.ThrowsAsync<PostAlreadyArchivedException>(() => postManager.ArchivePostAsync(fakePostId));
        }

        [Fact]
        public async Task Archive_post_throws_exception_when_error()
        {
            // Arrange
            var fakePostId = Guid.NewGuid();
            ...
            // Assert
            await Assert.ThrowsAsync<PostNotProcessedException>(() => postManager.ArchivePostAsync(fakePostId));
        }
    }
}